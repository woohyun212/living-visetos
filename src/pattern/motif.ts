 ...
 * 다이아몬드를 '모티프 형태'로 배치한다. 기술적 제약이 아니라 디자인 판단이다 —
 * 기울어진 격자보다 직교 배치가 가방 표면과 실루엣 오버레이에서 더 잘 읽혔고,
 * 다이아 리듬이라는 정체성은 모티프 형태 쪽이 담당한다. 따라서 격자 각도는
 * 더 이상 문법 파라미터가 아니며 `gridAngleDeg`는 제거되었다.
 */
