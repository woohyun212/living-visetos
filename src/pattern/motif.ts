 ...
 * 다이아몬드를 '모티프 형태'로 배치한다. 기술적 제약이 아니라 원본 충실도 판단이다.
 *
 * 실제 비세토스는 45° 사선을 교차시켜 만든 격자가 아니라, 심볼과 다이아몬드가 규칙적으로
 * 교대하는 배치에 가깝다. v0의 교차 사선 격자는 그 인상과 오히려 멀어 보였고, 정체성을
 * 만드는 것은 격자 각도가 아니라 '규칙적인 다이아몬드 + 교대'라고 판단해 직교 체커보드로
 * 옮겼다. 교대의 한쪽 자리는 가드레일("문자·로고 없음")에 따라 공식 마크가 아닌 자체
 * 추상 엠블럼(MOTIF_VARIANTS)이 채운다.
 *
