...
## 2. 모듈 분담 (2026-08-13 확정)

| 모듈 | 기능 | 담당 | 갈아 끼울 함수 | 완성 기준 (DoD) |
| --- | --- | --- | --- | --- |
| A 비전·캡처 | **F-01** | **PO** | `extractSeed()` + ADR-003 이행 | 조명 변화에도 안정적인 색·모션 추출, 마스크 경계 페더링, tasks-vision·로컬 모델로 오프라인 동작 |
| B 패턴 엔진 | **F-02** | 팀원① | `generateTile()` | 문법 파라미터화(간격·밀도·모티프), 시드 고정 시 재현 가능, L2 승격 연결 |
| C-1 오버레이 | **F-03** | 팀원② | `segLoop()` | 2D 합성→WebGL 셰이더 플레인 승격, 전환 연출(C-2와 협업) |
| C-2 3D 가방 | **F-04** | 팀원③ | `initBag()` | 실제 가방 GLTF 교체, L2 텍스처 크로스페이드 스왑 |
| D 결과물·웹 | **F-09** (+F-05·06 권장) | 팀원④ | (신규) | 대시보드(F-09)는 P2 — **P1인 F-05(QR 전송)·F-06(주문 목업) 선행 권장**, 8초 영상 인코딩 |
| E 앱 셸·통합 | — | **미정 (PO 겸직 제안)** | `btnAll` 로직 | ATTRACT→OWN 상태머신, 키오스크 화면 설계, 리허설 시나리오 |

> 미배정 확인 필요: **F-05(QR 전송, P1 — "심사위원 폰으로 전송" 킬러 장면)·F-06·E(통합)·클라우드.** 제안: F-05·06은 D 담당의 선행 과제로, E와 클라우드 조율은 PO 겸직.
